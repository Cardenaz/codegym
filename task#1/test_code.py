...
def fixed_sorting(sortfunc):
    ...
    for i in range(0, TEST_TIMES):
        rand_list = np.random.randint(rand_low,rand_high, size=rand_list_size).tolist()
...
def create_report(sortfunc_list):
    ...
    for k,sortfunc in enumerate(sortfunc_list):
        ...
        variable_dataframe.insert(k, sortfunc_names[k] ,variable_sorting(sortfunc))
        print(f'{newline}Testing: {sortfunc_names[k]} for fixed list size ({LIST_SIZE})')
        bench_dataframe.insert(k, sortfunc_names[k] ,fixed_sorting(sortfunc))
    ...
    for j,sortfunc in enumerate(sortfunc_names):
        ...
        min_time = min(bench_dataframe.iloc[:,j])
        max_time = max(bench_dataframe.iloc[:,j])
        avg_time = mean(bench_dataframe.iloc[:,j])
        med_time = median(bench_dataframe.iloc[:,j])
        sd_time = stdev(bench_dataframe.iloc[:,j])
        stat_sentence = \
        ...
        print(f'{newline}For {sortfunc_names[j]}:{newline} {stat_sentence} {newline}')
        ...
        for x in hist_plot:
            ...
            x.set_title(f"Time duration for {sortfunc_names[j]}")
    ...
    g=sns.lmplot(x='Input Size', y='value', data=grow_melted, hue='variable',legend=False, fit_reg=True,scatter_kws={"s": 10,"alpha":.5},line_kws={"lw":2,"alpha":0.5})#legend=False,
